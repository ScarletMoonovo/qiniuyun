...
#### [x] T005: 实时语音流服务（已简化为Simple-peer）
- **描述**: 基于Simple-peer实现WebRTC实时语音通话
- **优先级**: P0
- **预估工时**: 0.5天（简化后）
- **文件**: `src/utils/simplePeerVoiceService.ts`
- **依赖**: T002, T006
- **验收标准**:
  - ✅ Simple-peer WebRTC连接
  - ✅ 实时双向音频传输
  - ✅ 自动回声消除和噪声抑制
  - ✅ 连接状态管理
  - ✅ 简化的信令处理
...
#### [x] T006b: Simple-peer信令集成（已简化）
- **描述**: WebSocket服务集成Simple-peer信令
- **优先级**: P0
- **预估工时**: 0.2天（大幅简化）
- **文件**: `src/utils/socketService.ts`（已修改）
- **依赖**: T006
- **验收标准**:
  - ✅ Simple-peer信令统一处理
  - ✅ 自动ICE候选和SDP交换
