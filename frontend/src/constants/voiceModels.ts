...
// 按分类组织语音模型选项
export const getVoiceModelOptions = () => {
  const categories = ['传统音色', '双语音色', '特殊音色'];
  ...
  return categories.map(category => ({
    ...
    options: VOICE_MODELS
      ...
      .map(model => ({
        label: model.voice_name,
        value: model.voice_type, // 使用voice_type作为value
        category: model.category,
        voice_type: model.voice_type,
        url: model.url
      }))
  }));
};

// 根据voice_type获取语音模型信息
export const getVoiceModelByType = (voice_type: string): VoiceModel | undefined => {
  return VOICE_MODELS.find(model => model.voice_type === voice_type);
};